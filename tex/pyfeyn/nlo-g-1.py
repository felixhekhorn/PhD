...
vnuo = Vertex(2.5, 2, mark=CIRCLE).addLabel(r"$\nu_o,b$",displace=.3,angle=90)

Fermion(oAQ, vnuo).addStyle(pyx.style.linewidth(0.05*pyx.unit.w_cm)).addArrow()\
...
Fermion(vnuo,vmu).addStyle(pyx.style.linewidth(0.05*pyx.unit.w_cm)).addArrow()\
	.addParallelArrow(size=0.1, displace=0.06).addLabel(r"$-p_2-k_2$",angle=90,displace=.3)
Fermion(vmu,vnui).addStyle(pyx.style.linewidth(0.05*pyx.unit.w_cm)).addArrow()\
	.addParallelArrow(size=0.1, displace=0.06).addLabel(r"$p_1-k_1$",angle=90,displace=.65)
