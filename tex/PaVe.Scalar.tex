...
To compute $D_0(0,m^2,q^2,m^2,t,u,0,0,m^2,m^2)$ I neither suceeded with \cite{Bojak:2000eu} nor \cite{Denner:1991qq}, but one can use \cite[Box 11]{Ellis:2007qk}. The transformation from the notation in \cite{Ellis:2007qk} is given by
\begin{equation}
...
\begin{align}
\Rightarrow\, &iC_\epsilon\left(\frac{a^B}{(n-4)^2}+\frac{b^B}{n-4}+c^B + O(n-4)\right)\nonumber\\
 &\EqualClaim \frac{\mu^{4-n}}{(2\pi)^n} \frac{i\pi^{n/2}r_\Gamma}{\mu^{4-n}}\left(\frac{a^E}{(n-4)^2}+\frac{b^E}{n-4}+c^E + O(n-4)\right)
\end{align}
\begin{align}
\Rightarrow a^B &=a^E\\
b^B &= b^E-\frac 1 2 a^E\ln(m^2/\mu^2)\\
c^B &= c^E-a^E\frac{\pi^2}{48}+\frac{a^E}8\ln^2(m^2/\mu^2)-\frac{b^E} 2\ln(m^2/\mu^2)
\end{align}
So we get
\begin{align}
