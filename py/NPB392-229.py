...
# add a data point
def add(m2,proj,nlf,lambdaQCD,mu2,bjorkenX,y0,n):
	qL = "c" if 3 == nlf else ("b" if 4 == nlf else "t")
	fp = pathOut+"dF%s%s_dy_x-%g_%d.dat"%(proj,qL,-np.log10(bjorkenX),n)
	q2 = -10.
	qIn.put({"oArgs":(m2,q2,proj,nlf,xTilde,omega,deltax,deltay,), "nlf":nlf, "lambdaQCD": lambdaQCD, "mu2": mu2, "bjorkenX":bjorkenX, "y0":y0, "n":n, "fp":fp})

def addCharm(proj,bjorkenX,y0):
	m2 = 1.5**2
	nlf = 3
	lambdaQCD = 0.194
	mu2 = (4.,-1.,0.,)
...
def addRapG():
	addCharm(projT.G, .1, 2.)
	addCharm(projT.G, .01, 3.5)
	addCharm(projT.G, .001, 4.5)
	addCharm(projT.G, .0001, 5.5)

def addRapL():
	addCharm(projT.L, .1, 2.)
	addCharm(projT.L, .01, 3.5)
	addCharm(projT.L, .001, 4.5)
	addCharm(projT.L, .0001, 5.5)
