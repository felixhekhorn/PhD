...
c      character * (*) fname
c
      subroutine time(ctime)           ! returns a string with the time
      character * (*) ctime
      return
      end
c
c      subroutine idate(iday,imon,iyear)! returns integers for day, month, year
c
...
*   10 FORMAT(4X,2E10.3)
   15 FORMAT(/' AVG =',1PE11.4,4X,' RMS =',1PE11.4,
     &        ' INTEGRAL =',1PE11.4,/)
*   20 FORMAT(' ENTRIES=',I6,4X,'UNDERSCORE=',I5,4x,'OVERSCORE=',I5,//)
      END
...
C-- dots - as many as the value of HINT2(n), distributed according to wgt
      ...
      DO 3, I=1,NBIN2(1,N)
      DO 3, J=1,NBIN2(2,N)
      IF(HIST2(N,I,J).EQ.0.) GO TO 3
      NDOTS=INT(HIST2(N,I,J))
      call randa(1,rnn)
      REST=HIST2(N,I,J)-FLOAT(NDOTS)
      IF(sngl(RN(1)).LT.REST) NDOTS=NDOTS+1
      DO ND=1,NDOTS
      call randa(2,rn)
      X=XHIS2(N,I)+(2.*sngl(RN(1))-1.)*HDEL2(1,N)
      Y=YHIS2(N,J)+(2.*sngl(RN(2))-1.)*HDEL2(2,N)
      WRITE(99,*) X,Y           
      ENDDO
      NTOT=NTOT+NDOTS
  3   CONTINUE
...
c                   
      REAL LAB0,LABS
      CHARACTER TITLE*50,BOOK*3,CTIME*8,SCALE*3
...
c      CALL IDATE(IMON,IDAY,IYEAR)
      CALL TIME(CTIME)
      IFRAME=0
      WRITE(99,71) IYEAR,IMON,IDAY,CTIME(1:5)
   71 FORMAT(4X,' (   19',I2,' -',I2,' -',I2,1X,A5/)
      INI=1
      ENDIF
      IF(SCA.EQ.'REF') THEN
        IFRAME=0 
      ...
      IF(IFRAME.GT.IFRMAX.OR.N.NE.NOLD.OR.M.NE.MOLD) THEN
        IFRAME=1
        WRITE(99,202)
        WRITE(99,1) IMON,IDAY,CTIME(1:5)
  1     FORMAT(' ( SET FONT DUPLEX',/,'  SET TITLE SIZE 2',/,
     +      ' TITLE 12.8 9 ANGLE -90 ','" MLM   ',I2,'-',I2,1X,A5,'"')
      ENDIF
      IF(IFRAME.EQ.1) THEN
        I=1
        J=1
