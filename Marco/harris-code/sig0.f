      ...
      PARAMETER (AEM=1D0/137.04D0)
      PARAMETER (XME2=2.6D-7)
      PARAMETER (AEM2=AEM*AEM)
...
C. Y
      ...
      OMY=1D0-Y
C. Q2
      Q2U=MIN(Y*SS-4*XM2,Q2MAX)
      Q2L=MAX(XME2*Y*Y/(1-Y),Q2MIN)
      XJACQ2=Q2U-Q2L
      Q2=Q2L+XJACQ2*XX(2)
...
C. CALCULATE STRONG COUPLING
      ALFAS=ALPHAS(IALPHAS,XMUR2,XLQCD5,NEFF)
C. FILL PDFS
      CALL PDF(XI,XMUF2)
C.
      IF ( NCOUNT.EQ.0 ) THEN
        ...
        WRITE(*,*) ' '
        WRITE(*,*) 'INTEGRATING LO ...'
        WRITE(*,*) ' '
      END IF
C. CALL STRUCTURE FUNCTIONS
      TF=Q2*XKGP*ALFAS*EH2*N*CF*BETA*GLUE/(2D0*PI*SP*XI)
