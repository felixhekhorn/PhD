...
LOK=
  ...
  (2 t1^3 u1^3)) zeta2 + 
  (-2 m^2 s + t1^2 + u1^2) 
  (Log[-t1/m^2]+ Log[-u1/m^2])/(4 t1 u1) -  ((24 m^4 - 3 s^2 + 2 t1 u1 - 
  ...
  BQED ((-3 zeta2 +4 Log[muf^2/m^2] Log[-t1/m^2] + 
  Log[t1/u1]^2 + 2 Log[t1/u1] Log[x] - Log[x]^2 - 
  Li2[1 - t1/(u1 x)] + 
  2 Li2[1 - u1/(t1 x)])/4 + 
...
check = Expand[help /. {s->-t1-u1,t->t1+m^2,u->u1+m^2}];
