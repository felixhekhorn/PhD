...
c      jacob = (mt1up-mt1do) * (mu1up-mu1do)
      u1 = -s-t1
      jacob = (mt1up - mt1do)
      cgsv = m2/s**2/4.d0/pi * jacob * 
     1     (svok(s,t1,u1,m) + svqed(s,t1,u1,m))
cc      cgsv = m2/s**2/4.d0/pi * jacob * svok(s,t1,u1,m)  
      return
      end
c ***
...
c
     ...
     1     2*m4*(t12 + 2*t1*u1 + 5*u12))*li2u)/(32.*t1*u13)
      part7 = -((4*m**8*s*(5*t12 + 6*t1*u1 + 5*u12) - 
     ...
     8     lnt)/(64.*s*t**3*t12*u1)
      part8 = -((4*m**8*s*(5*t12 + 6*t1*u1 + 5*u12) + 
