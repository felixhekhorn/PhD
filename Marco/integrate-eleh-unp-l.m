...
color=1;
n=4+eps;
mixed=color*2*(<<pqm1m3-unp-l + <<pqm1m4-unp-l +
               <<pqm2m3-unp-l + <<pqm2m4-unp-l );
...
help=Expand[step2 /. {u7/ts/us -> (-sp-u1p-us)/ts/us,
                      ...
                      ts^2/s5/u7 -> (u1p+s5+u7)^2/s5/u7,
                      ts/s5/u7 -> (u1p+s5+u7)/s5/u7,
                      ts^2/s5/us -> (s5-sp-us)^2/s5/us,
...
help = Expand[I00 unint /. {1/s5/ts ->I11tss5/I00,
                    ...
                    1/us/ts ->I11tsus/I00,
                    1/s5/u7 ->I11u7s5/I00,
                    ts^2/u7 ->IM21tsu7/I00,
                    ts^2/us ->IM21tsus/I00,
                    s5/ts   ->I1M1tss5/I00,
                    ts/s5   ->IM11tss5/I00,
                    us/ts   ->I1M1tsus/I00,
                    ts/us   ->IM11tsus/I00,
...
IM21tsu7 = Pi * (  (-2*s4*(q2 - s - t1)*(2*q2^2 + 2*m2*(-q2 + s) + s*(s + t1) + (s - t1)*u1 - 
                          q2*(3*s + 2*t1 + u1)))/((m2 + s4)*(4*q2*(m2 + s4) - (s + u1)^2)) +
                     -(((2*m2*(q2 - s - u1) + s4*(2*q2 - s - u1))*(6*m2^2*(q2 - s)^2 + 
                            (-q2 + s + t1)^2*((-2*q2 + s)^2 + 2*q2*t1) - 2*(2*q2 - s)*(q2 - s - t1)*
                            (q2 - s + 2*t1)*u1 + ((q2 - s)^2 + 4*(q2 - s)*t1 + t1^2)*u1^2 + 
                             2*m2*(-5*q2^3 + q2^2*(13*s + 4*t1 + 3*u1) + 
                             3*s*(s*(s + t1) + (s - t1)*u1) + q2*(-11*s^2 - 7*s*t1 + t1^2 - 6*s*u1 + 
                             3*t1*u1))))/((m2 + s4)*(-4*q2*(m2 + s4) + (s + u1)^2)^2)) +
                    ( (2*(m2 + s4)*(2*m2*(q2 - s)*(q2 - s - u1) + t1*(2*q2*s4 - u1*(s + u1)))^2)/
                              (s4*(-4*q2*(m2 + s4) + (s + u1)^2)^(5/2))  -
                      (-4*(m2 + s4)*(m2*(q2 - s)^2 + q2*t1*(-q2 + s + t1) + (q2 - s)*t1*u1)*
                               (q2*s4*t1 + m2*(-q2 + s + u1)^2))/(s4*(-4*q2*(m2 + s4) + (s + u1)^2)^(5/2))
                      ) *  Log[(2*m2*(q2 - s - u1) + s4*(2*q2 - s - u1 + 
                               Sqrt[-4*q2*(m2 + s4) + (s + u1)^2]))/
                               (2*m2*(q2 - s - u1) - s4*(-2*q2 + s + u1 + 
                               Sqrt[-4*q2*(m2 + s4) + (s + u1)^2]))]);
...
IM21tsus = Pi * ( (2*s4*(q2 - s - t1)*(2*q2^2 + 2*m2*(-q2 + s) + s*(s + t1) + (s - t1)*u1 - 
                          q2*(3*s + 2*t1 + u1)))/((m2 + s4)*(4*q2*(m2 + s4) - (s + u1)^2)) +
                      -(((2*m2*q2 + s4*(2*q2 - s - u1))*(6*m2^2*(q2 - s)^2 + 
                           (-q2 + s + t1)^2*((-2*q2 + s)^2 + 2*q2*t1) - 2*(2*q2 - s)*(q2 - s - t1)*
                           (q2 - s + 2*t1)*u1 + ((q2 - s)^2 + 4*(q2 - s)*t1 + t1^2)*u1^2 + 
                            2*m2*(-5*q2^3 + q2^2*(13*s + 4*t1 + 3*u1) + 
                            3*s*(s*(s + t1) + (s - t1)*u1) + q2*(-11*s^2 - 7*s*t1 + t1^2 - 6*s*u1 + 
                    ...
                    (  (2*(m2 + s4)*(-4*q2^3 + 2*m2*q2*(q2 - s) + s*s4*(s + u1) + 
                            q2^2*(8*s + 6*t1 + 4*u1) - q2*(4*s^2 + 6*s*t1 + 2*t1^2 + 5*s*u1 + 
                            2*t1*u1 + u1^2))^2)/(s4*(-4*q2*(m2 + s4) + (s + u1)^2)^(5/2))  -
                        (-4*q2*(m2 + s4)*(m2*q2 + s4*t1)*(m2*(q2 - s)^2 + q2*t1*(-q2 + s + t1) + 
                            (q2 - s)*t1*u1))/(s4*(-4*q2*(m2 + s4) + (s + u1)^2)^(5/2))
                      ) *  Log[(2*m2*q2 + s4*(2*q2 - s - u1 + Sqrt[-4*q2*(m2 + s4) + (s + u1)^2]))/
                               (2*m2*q2 - s4*(-2*q2 + s + u1 + Sqrt[-4*q2*(m2 + s4) + (s + u1)^2]))] );
...
I11u7s5 = (2*Pi*(m2 + s4)*(Log[2*m2*s*(-q2 + s + u1) + 
            ...
            Log[2*m2*s*(-q2 + s + u1) + s4*(-(q2*s) + 2*q2*s4 - s4*u1 + 
