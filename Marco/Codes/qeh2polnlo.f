...
C  COEFFICIENT FUNCTION FOR q gamma -> Q Qbar q IN NLO (POLARIZED)   C
C                           EH**2 CONTRIBUTION                       C
C             STATUS :  19/AUG/99                                    C
C                       HAT MOMENTA INCLUDED                         C
C                       CHECKED                                      C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
      program polhq
...
c      
     ...
     1     (s**2 + 2*s*t1 + 2*t1**2)*(2*s + 2*t1 + u1)*
     2     (2*m2*s - t1*u1)/(2*t1**2*(s + t1)**2*u1**2)
      return
      end
c ***
...
c      
      ...
      m4 = m2*m2
      s4 = s+t1+u1
      part1 = cf *(s**2 + 2*s*t1 + 2*t1**2)*(2*s + 2*t1 + u1)*
     1     (2*m2*s - t1*u1)*Log(s4**2/(m2*(m2 + s4)))/
     2     (2*t1**2*(s + t1)**2*u1**2)
      part2 = cf * 0.d0 
...
c      
      ...
      u16 = u13*u13
      s4 = s+t1+u1
      part1old = cf *s4*(-8*m4*s4 - 8*m2*s4*s4 - 16*m4*s3*t1 - 
     1     16*m2*s3*s4*t1 - 12*m4*s2*t12 - 10*m2*s2*s4*t12 - 
     2     4*m4*s3*u1 - 4*m2*s3*s4*u1 - 8*m4*s2*t1*u1 + 
     3     4*m2*s3*t1*u1 - 8*m2*s2*s4*t1*u1 + 4*s3*s4*t1*u1 - 
     4     8*m4*s*t12*u1 + 6*m2*s2*t12*u1 + 2*s3*t12*u1 - 
     5     6*m2*s*s4*t12*u1 + 7*s2*s4*t12*u1 + 6*m2*s*t13*u1 + 
     6     4*s2*t13*u1 + 4*s*s4*t13*u1 + 2*s*t14*u1 + 
     7     2*m2*s2*t1*u1**2 + 2*s2*s4*t1*u12 + 2*m2*s*t12*u12 + 
     8     2*s2*t12*u1**2 + 3*s*s4*t12*u1**2 + 4*m2*t1**3*u12 + 
     9     5*s*t13*u1**2 + 2*s4*t13*u12 + 2*t14*u12 + t13*u13)/
     1     (2*(m2 + s4)*t12*(s + t1)**2*u12*(s + u1))
      part1= -(cf *s4*(8*m4*ss4 + 8*m2*ss4*s4 + 16*m4*s3*t1 + 
     1     16*m2*s3*s4*t1 + 12*m4*s2*t12 + 
     2     10*m2*s**2*s4*t1**2 + 4*m4*s3*u1 + 4*m2*s3*s4*u1 + 
     3     8*m4*s**2*t1*u1 - 4*m2*s**3*t1*u1 + 8*m2*s2*s4*t1*u1 - 
     4     4*s3*s4*t1*u1 + 8*m4*s*t1**2*u1 - 6*m2*s2*t12*u1 - 
     5     2*s3*t1**2*u1 + 6*m2*s*s4*t1**2*u1 - 7*s2*s4*t12*u1 - 
     6     6*m2*s*t1**3*u1 - 4*s2*t13*u1 - 4*s*s4*t13*u1 - 
     7     2*s*t14*u1 - 2*m2*s2*t1*u12 - 2*s2*s4*t1*u12 - 
     8     2*m2*s*t12*u12 - 2*s2*t1**2*u1**2 - 3*s*s4*t12*u12 - 
     9     4*m2*t13*u12 - 5*s*t13*u12 - 2*s4*t13*u12 - 
     1     2*t14*u12 - t13*u13))/
     2     (2.*(m2 + s4)*t12*(s + t1)**2*u12*(s + u1))
       part2 = cf * s * Log(m2/(m2 + s4))/(u1*(s + u1))
       part3old =  cf *(2*s + 2*t1 + u1)*(2*m2*s - t1*u1)*
     1      Log((m2 + s4)*t12*u12/(m2*(s + t1)**2*(s + u1)**2))/
     2      (2*t12*u12)
       part3 = - (cf *(2*s + 2*t1 + u1)*(-2*m2*s + t1*u1)*
     1      Log(((m2 + s4)*t12*u12)/(m2*(s + t1)**2*(s + u1)**2)))/
     2      (2.*t12*u12)
C... HAT CONTRIBUTION
       hat = -((cf*s4*(s2 + 2*s*t1 + 2*t12)*
     1      (2*m2*s - t1*u1))/(t12*(s + t1)**2*u12))
C... SUM
       qeh2finite=(part1+part2+part3)+hat
       return
       end
