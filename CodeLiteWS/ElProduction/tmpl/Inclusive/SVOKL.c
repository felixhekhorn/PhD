...
dbl c5 = ((64*Power(m2,2)*q2)/(s*t1*u1) + (24*q2*(q2 - t1 - u1))/Power(t1 + u1,2) + 32*m2*q2*(-(1/(t1*u1)) - Power(t1 + u1,-2)))/beta;
r += c5*v5;
dbl v6 = Li2(chi) + ln(1 - chi)*ln(chi);
dbl c6 = ((64*Power(m2,2)*q2)/(s*t1*u1) + (32*q2*(q2 - t1 - u1))/Power(t1 + u1,2) + 32*m2*q2*(-(1/(t1*u1)) - 2/Power(t1 + u1,2)))/beta;
r += c6*v6;
dbl v7 = Li2(-(chi/chiq)) + Li2(-(chi*chiq)) + ln(chi/chiq)*ln((chi + chiq)/chiq) + ln(chi*chiq)*ln(1 + chi*chiq) - (ln(chi)*(ln(-(t1/m2)) + ln(-(u1/m2))))/2.;
dbl c7 = ((16*m2*q2)/(t1*u1) - (32*Power(m2,2)*q2)/(s*t1*u1) - (8*q2*s)/Power(t1 + u1,2))/beta;
r += c7*v7;
dbl v8 = Power(ln(-(t1/m2)),2) + Power(ln(-(u1/m2)),2);
dbl c8 = (-8*m2*q2)/Power(t1 + u1,2) + (4*q2*(q2 - t1 - u1))/Power(t1 + u1,2);
r += c8*v8;
dbl v9 = ln(-(t1/m2))*ln(-(u1/m2));
dbl c9 = (8*q2*(q2 - t1 - u1))/Power(t1 + u1,2) + 16*m2*q2*(-(1/(t1*u1)) + Power(t1 + u1,-2));
r += c9*v9;
dbl v10 = ln(-(t1/m2));
dbl c10 = (2*q2)/u1 - (4*q2*Power(t1,2)*(Power(t1,2) + (q2 + t1)*u1))/((m2 + t1)*(q2 + t1)*u1*Power(t1 + u1,2)) - (2*q2*(q2 - t1)*Power(q2*(-t1 + u1) + t1*(t1 + u1),2))/((-4*m2*q2 + Power(q2 - t1,2))*(q2 + t1)*u1*Power(t1 + u1,2));
r += c10*v10;
dbl v11 = ln(-(u1/m2));
dbl c11 = (2*q2)/t1 - (4*q2*Power(u1,2)*(q2*t1 + u1*(t1 + u1)))/(t1*(m2 + u1)*(q2 + u1)*Power(t1 + u1,2)) - (2*q2*(q2 - u1)*Power(q2*(t1 - u1) + u1*(t1 + u1),2))/(t1*(-4*m2*q2 + Power(q2 - u1,2))*(q2 + u1)*Power(t1 + u1,2));
r += c11*v11;
dbl v12 = Power(ln(chi),2);
dbl c12 = (-10*m2*q2)/(t1*u1) + (10*q2*(q2 - t1 - u1))/Power(t1 + u1,2) + ((-16*Power(m2,2)*q2)/(s*t1*u1) - (10*q2*s)/Power(t1 + u1,2) + 8*m2*q2*(1/(t1*u1) + 3/Power(t1 + u1,2)))/beta;
r += c12*v12;
dbl v13 = ln(chi);
dbl c13 = beta*((-2*q2*s)/(t1*u1) - (2*q2*Power(s,2)*Power(t1 - u1,2))/((4*m2 - s)*t1*u1*Power(t1 + u1,2)));
r += c13*v13;
dbl v14 = Power(ln(chiq),2);
dbl c14 = (-6*q2*s)/Power(t1 + u1,2) + 2*m2*q2*(5/(t1*u1) - 4/Power(t1 + u1,2)) + ((-8*m2*q2)/(t1*u1) + (16*Power(m2,2)*q2)/(s*t1*u1) + (4*q2*(q2 - t1 - u1))/Power(t1 + u1,2))/beta;
r += c14*v14;
dbl v15 = ln(chiq);
dbl c15 = -((betaq*q2*(-2*q2 + t1 + u1))/(t1*u1)) - (betaq*q2*(q2 - t1)*Power(q2*(-t1 + u1) + t1*(t1 + u1),2))/((-4*m2*q2 + Power(q2 - t1,2))*t1*u1*Power(t1 + u1,2)) - (betaq*q2*(q2 - u1)*Power(q2*(t1 - u1) + u1*(t1 + u1),2))/(t1*(-4*m2*q2 + Power(q2 - u1,2))*u1*Power(t1 + u1,2));
r += c15*v15;
dbl v16 = zeta2;
dbl c16 = (-8*q2*s)/Power(t1 + u1,2) + 8*m2*q2*(3/(t1*u1) - 4/Power(t1 + u1,2)) + ((32*Power(m2,2)*q2)/(s*t1*u1) + (20*q2*(q2 - t1 - u1))/Power(t1 + u1,2) + 16*m2*q2*(-(1/(t1*u1)) - 3/Power(t1 + u1,2)))/beta;
r += c16*v16;
