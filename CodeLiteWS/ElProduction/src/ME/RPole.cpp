...
dbl u1 = s4-sp-t1;
return (4*(Power(sp,2) + 2*sp*t1 + 2*Power(t1,2))*(2*m2*Power(sp,2) + 2*q2*t1*(sp + t1) - sp*t1*u1)*(2*Power(sp + t1,2) + 3*(sp + t1)*u1 + 2*Power(u1,2))*Log(Power(s4,2)/(m2*(m2 + s4))))/(s4*sp*Power(t1,2)*Power(sp + t1,2)*Power(u1,2));
}
...
dbl u1 = s4-sp-t1;
return (-4*(Power(sp,2) + 2*sp*t1 + 2*Power(t1,2))*(2*m2*Power(sp,2) + 2*q2*t1*(sp + t1) - sp*t1*u1)*(2*Power(sp + t1,2) + 3*(sp + t1)*u1 + 2*Power(u1,2)))/(s4*sp*Power(t1,2)*Power(sp + t1,2)*Power(u1,2));
}