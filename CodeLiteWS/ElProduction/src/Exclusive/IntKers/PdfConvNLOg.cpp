...
dbl PdfConvNLOg::cg1() const {
    ...
    // hard contributions
    {
        ...
        r += jacxE*jacyE*jacTheta1*jacTheta2 * f*vsE.beta5/(1.-yE) * 1./(1.-xE)/(1.+yE) *    Rp     (m2,q2,sp,xE,yE,Theta1,Theta2);
        r -= jacxC*jacyE*jacTheta1*jacTheta2 * f*    beta /(1.-yE) * 1./(1.-xC)/(1.+yE) *    RpxC   (m2,q2,sp,yE,Theta1,Theta2);
        r -= jacxE*jacyC*jacTheta1*jacTheta2 * f*vsE.beta5/2.      * 1./(1.-xE)/(1.+yC) * CA*ROKpyC (m2,q2,sp,xE,Theta1,Theta2);
...
dbl PdfConvNLOg::cgBarR1() const {
    ...
    if (0 == this->BpQED)
        throw invalid_argument("need to set all arguments!");
    cdbl b0 = (11.*CA - 2.*(nlf))/3.;
    cdbl s = sp + q2;
    cdbl beta = sqrt(1. - 4.*m2/s);
    cdbl t1 = -.5*sp*(1. - beta*cos(Theta1));
    cdbl f = 8. * Kggg*NC*CF * 1./(4.*sp);
    //printf("%e %e %e %e\t%e\n",m2,q2,sp,t1,BpQED(m2,q2,sp,t1));
    dbl r = jacTheta1 * f * b0 * BpQED(m2,q2,sp,t1) * beta*sin(Theta1)/(16.*M_PI);
    // norm to cg1
    r *= (m2/4.*M_PI) * /** @todo ??? */ 1.2732396;
    if (!isfinite(r)) return 0.;
    return r;
}
