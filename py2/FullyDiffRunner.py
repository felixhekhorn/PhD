...
from LeptoProduction import FullyDiffLeptoProduction, Flags, DynamicScaleFactors
import Util

# compute all data points
class FullyDiffRunner:
	__qIn = JoinableQueue()
	__qOut = Queue()
	def add(self,e):
		"""
...
def _threadWorker(qIn, qOut):
	while True:
		...
		if p.has_key("xTilde"): o.setXTilde(p["xTilde"])
		...
		if p.has_key("activateHistograms"):
			for k in p["activateHistograms"]:
				o.activatedHistograms(*k)
		# leptonic setter
